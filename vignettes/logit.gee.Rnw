...
\include{zinput}
%\VignetteIndexEntry{logit.gee: Generalized Estimating Equation for Logistic Regression}
...
specified.  If the ``working'' correlation matrix is correctly specified, GEE models will give more efficient estimates of the parameters.     
...
dependent variable for unit $i$ at time $t$ which takes the value of
either 0 or 1.  $Y_{i}$ is a vector or cluster of correlated data
where $y_{it}$ is correlated with $y_{it^\prime}$ for some or all
$t, t^\prime$.  In the GEE model, we must specify a mean function, a
variance function, and the structure of the correlation matrix of the within-cluster
observations.  Observations are assumed to be correlated within each
cluster but not across clusters.  


\begin{itemize}

\item The \emph{stochastic component} is given by the joint distribution
...
\begin{eqnarray*}
Y_{it} &\sim& \mathrm{Bernoulli}(y_{it} \mid \pi_{it}) \\
    &=& \pi_{it}^{y_{it}} (1-\pi_{it})^{1-y_{it}}
\end{eqnarray*}
where $\pi_{it}=\Pr(Y_{it}=1)$ for $t=1,...,T$.  The correlations
within each unit $i$ are modeled by defining the structure of a $T \times T$
``working'' correlation matrix, which is specified by the user
\textit{a priori}.  Note that the model assumes correlations within $i$ but independence across $i$.  The ``working'' correlation matrix then enters
the variance term for each $i$, given by:
\begin{equation*}
V_{i} = \phi \, A_{i}^{\frac{1}{2}} R_{i}(\alpha) A_{i}^{\frac{1}{2}}
\end{equation*}
where $A_{i}$ is a $T \times T$ diagonal matrix with $var(\pi_{it}) = \pi_{it} (1-\pi_{it})$ as the $t$th diagonal element, $R_{i}(\alpha)$ is the ``working'' correlation matrix, and $\phi$ is a scale parameter.  The parameters are then estimated via a quasi-likelihood approach.
...
\item The predicted values ({\tt qi\$pr}) are draws from the Binomial
  distribution with mean equal to the simulated expected value $\pi_{c}$.
...
\item In conditional prediction models, the average predicted treatment
  effect ({\tt att.pr}) for the treatment group is
    \begin{equation*} \frac{1}{\sum_{i=1}^n \sum_{t=1}^T tr_{it}}\sum_{i:tr_{it}=1}^n \sum_{t:tr_{it}=1}^T \left\{ Y_{it}(tr_{it}=1) -
      \widehat{Y_{it}(tr_{it}=0)}\right\},
    \end{equation*}
    where $tr_{it}$ is a binary explanatory variable defining the
    treatment ($tr_{it}=1$) and control ($tr_{it}=0$) groups.  Variation in
    the simulations are due to uncertainty in simulating
    $\widehat{Y_{it}(tr_{it}=0)}$, the counterfactual predicted value of
    $Y_{it}$ for observations in the treatment group, under the
    assumption that everything stays the same except that the
    treatment indicator is switched to $tr_{it}=0$.
\end{itemize}
...
may view.  For example, if you run \texttt{z.out <- zelig(y \~\, x,
  model = "logit.gee", id, data, corstr)}, then you may examine the available
...
\begin{verse}
Patrick Lam. 2007. ``logit.gee: Generalized Estimating Equation for Logistic Regression,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.

\end{verse}
\input{citeZelig}

\subsection* {See also}
The logit function is part of the base packages by William N. Venable \citep{VenRip02}. Advanced users may wish to refer to \texttt{help(glm)} and \texttt{help(family)}, as well as \cite{McCNel89}. Robust standard errors are implemented via sandwich package by Achim Zeileis \citep{Zeileis04}.Sample data are a selection of $2,000$ observations from \cite{KinTomWit00}
